    ...
    def __init__(self,
        ...
        if down_t > 0:
            for i in range(down_t):
                block = nn.Sequential(
                    # nn.Conv1d(input_emb_width if i == 0
                    # else width, width, filter_t, stride_t,
                    # pad_t, padding_mode='replicate'),
                    nn.Conv1d(input_emb_width if i == 0 else width, width,
                              filter_t, stride_t, pad_t),
        ...
        if down_t > 0:
            ...
            for i in range(down_t):
                block = nn.Sequential(
                    ...
                    nn.ConvTranspose1d(
                        width, input_emb_width if i == (down_t - 1) else width,
                        filter_t, stride_t, pad_t))
                blocks.append(block)
        self.model = nn.Sequential(*blocks)
...
class Encoder(nn.Module):
    ...
    def __init__(self, input_emb_width, output_emb_width, levels, downs_t,
        ...
        level_block = lambda \
            level, down_t, stride_t: EncoderConvBlock(  # noqa: E731
