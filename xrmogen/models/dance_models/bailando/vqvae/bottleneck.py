...
class BottleneckBlock(nn.Module):
    ...
    def init_k(self, x):
        _, emb_width, k_bins = self.mu, self.emb_width, self.k_bins
        self.init = True
    ...
    def update_k(self, x, x_l):
        ...
        with t.no_grad():
            ...
            self.k = usage * (
                self.k_sum.view(k_bins, emb_width) /
                self.k_elem.view(k_bins, 1)) + \
                (1 - usage) * _k_rand
    ...
    def preprocess(self, x):
        ...
        if x.shape[-1] == self.emb_width:
            ...
            prenorm = (
                t.norm(x1 - t.mean(x1)) / np.sqrt(np.prod(x1.shape))) + (
            ...
            assert False, \
                f'Expected {x.shape[-1]} to be' +\
                f' (1 or 2) * {self.emb_width}'
        return x, prenorm
    ...
    def quantise(self, x):
        ...
        k_w = self.k.t()
        distance = t.sum(
            x**2, dim=-1, keepdim=True) - 2 * t.matmul(x, k_w) + t.sum(
                k_w**2, dim=0, keepdim=True)  # (N * L, b)
    ...
    def forward(self, x, update_k=True):
        ...
        return x_l, x_d, commit_loss, dict(
            fit=fit, pn=prenorm, **update_metrics)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, l_bins, emb_width, mu, levels):
        super().__init__()
        self.levels = levels
        level_block = lambda level: BottleneckBlock(  # noqa: E731
            l_bins, emb_width, mu)
    ...
    def decode(self, zs, start_level=0, end_level=None):
        ...
        xs_quantised = [
            ...
            for (level_block,
                 z) in zip(self.level_blocks[start_level:end_level], zs)
        ]
        return xs_quantised
