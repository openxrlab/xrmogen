...
class SepVQVAER(nn.Module):
    ...
    def __init__(self, hps):
        ...
        self.vqvae_down = VQVAER(hps.down_half,
                                 len(smpl_down) * self.chanel_num)
    ...
    def decode(self, zs, start_level=0, end_level=None, bs_chunks=1):
        ...
        x = torch.zeros(b, t, (cup + cdown) // self.chanel_num,
                        self.chanel_num).cuda()
        x[:, :, smpl_up] = xup.view(b, t, cup // self.chanel_num,
                                    self.chanel_num)
        x[:, :, smpl_down] = xdown.view(b, t, cdown // self.chanel_num,
                                        self.chanel_num)
    ...
    def encode(self, x, start_level=0, end_level=None, bs_chunks=1):
        b, t, c = x.size()
        zup = self.vqvae_up.encode(
            x.view(b, t, c // self.chanel_num,
                   self.chanel_num)[:, :, smpl_up].view(b, t, -1), start_level,
            end_level, bs_chunks)
        zdown = self.vqvae_down.encode(
            x.view(b, t, c // self.chanel_num,
                   self.chanel_num)[:, :, smpl_down].view(b, t, -1),
    ...
    def forward(self, x, phase='motion vqvae'):
        ...
        xout = torch.zeros(b, t, (cup + cdown) // self.chanel_num,
                           self.chanel_num).cuda().float()
        xout[:, :, smpl_up] = x_out_up.view(b, t, cup // self.chanel_num,
                                            self.chanel_num)
        xout[:, :, smpl_down] = x_out_down.view(b, t, cdown // self.chanel_num,
                                                self.chanel_num)

        if phase == 'motion vqvae':
            return xout.view(b, t, -1), 0.5 * (loss_down + loss_up), None
        else:
