...
- the initial stem consists of a
combination of token encoding and a positional encoding
- the meat of it is a uniform sequence of
Transformer blocks
    - each Transformer is a sequential combination of a
    1-hidden-layer MLP block and a self-attention block
    - all blocks feed into a central residual pathway
    similar to resnets
- the final decoder is a linear projection into a
...
class CrossCondGPT(nn.Module):
    ...
    def sample(self, xs, cond, shift=None):
        ...
        for k in range(cond.size(1)):
            x_cond_up = x_up if x_up.size(1) <= block_size else x_up[:, -(
                block_shift + (k - block_size - 1) %
                (block_size - block_shift + 1)):]
            x_cond_down = x_down if x_down.size(
                1) <= block_size else x_down[:, -(
                    block_shift + (k - block_size - 1) %
                    (block_size - block_shift + 1)):]  # crop context if needed

            cond_input = cond[:, :k + 1] if k < block_size else cond[:, k - (
                block_shift + (k - block_size - 1) %
                (block_size - block_shift + 1)) + 1:k + 1]
    ...
    def forward(self, idxs, cond, targets=None):
        ...
        logits_up, logits_down, loss_up, loss_down = self.gpt_head(
            feat, targets)
...
class CausalCrossConditionalSelfAttention(nn.Module):
    ...
    def forward(self, x, layer_past=None):
        ...
        # in batch and move head forward to be the batch dim
        ...
        att = att.masked_fill(self.mask[:, :, :t, :t].repeat(1, 1, 3, 3) == 0,
                              float('-inf'))
...
class CrossCondGPTBase(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.pos_emb = nn.Parameter(
            torch.zeros(1, config.block_size * 3, config.n_embd))
    ...
    def configure_optimizers(self, train_config):
        ...
        that won't (biases, and layernorm/embedding weights). We are then
    ...
    def forward(self, idx_up, idx_down, cond):
        ...
        token_embeddings_up = self.tok_emb_up(
            idx_up)  # each index maps to a (learnable) vector
        token_embeddings_down = self.tok_emb_down(
            idx_down)  # each index maps to a (learnable) vector
        token_embeddings = torch.cat(
            [self.cond_emb(cond), token_embeddings_up, token_embeddings_down],
            dim=1)
...
class CrossCondGPTHead(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.block_size = config.block_size
        self.head_up = nn.Linear(
            config.n_embd, config.vocab_size_up, bias=False)
        self.head_down = nn.Linear(
            config.n_embd, config.vocab_size_down, bias=False)
    ...
    def configure_optimizers(self, train_config):
            ...
            for pn, p in m.named_parameters():
                ...
                if pn.endswith('bias'):
                    ...
                    no_decay.add(fpn)
                        ...
                        m, whitelist_weight_modules):
                    # weights of whitelist modules will
                    # be weight decayed
                    decay.add(fpn)
                elif pn.endswith('weight') and isinstance(
                        m, blacklist_weight_modules):
                    # weights of blacklist modules will
                    # NOT be weight decayed
                    no_decay.add(fpn)
        ...
        assert len(inter_params) == 0,\
            f'parameters {str(inter_params)} made it into' +\
    ...
    def forward(self, x, targets=None):
        ...
        if targets is not None:
            ...
            loss_up = F.cross_entropy(
                logits_up.view(-1, logits_up.size(-1)), targets_up.view(-1))
            loss_down = F.cross_entropy(
                logits_down.view(-1, logits_down.size(-1)),
                targets_down.view(-1))
