...
def load_train_data_aist(cfg):
    ...
    external_wav_rate = cfg.external_wav_rate if hasattr(
        cfg, 'external_wav_rate') else None
    ...
    for fname in fnames:
        ...
        with open(path) as f:
            ...
            if external_wav is not None:
                wav_path = os.path.join(external_wav,
                                        fname.split('_')[-2] + '.json')

                with open(wav_path) as ff:
                    sample_dict_wav = json.loads(ff.read())
                    np_music = np.array(sample_dict_wav['music_array']).astype(
                        np.float32)
            ...
            if not rotmat:
                ...
                np_dance = np_dance - np.tile(
                    root,
                    (1, 24))  # Calculate relative offset with respect to root
                np_dance[:, :3] = root

            music_sample_rate = external_wav_rate \
                if external_wav is not None else 1
            ...
            if interval is not None:  # just sample a piece of music
                ...
                for i in range(0, seq_len, move):
                    ...
                    music_sub_seq = np_music[i_sample:i_sample +
                                             interval_sample]
                    dance_sub_seq = np_dance[i:i + interval]

                    if len(music_sub_seq) == interval_sample and len(
                            dance_sub_seq) == interval:
...
def load_test_data_aist(cfg):
    ...
    for fname in fnames:
        ...
        with open(path) as f:
            ...
            np_music = np.array(sample_dict['music_array']).astype(np.float32)
            if external_wav is not None:
                # using music features from external files
                wav_path = os.path.join(external_wav,
                                        fname.split('_')[-2] + '.json')
                with open(wav_path) as ff:
                    sample_dict_wav = json.loads(ff.read())
            ...
            if 'dance_array' in sample_dict:
                np_dance = np.array(sample_dict['dance_array']).astype(
                    np.float32)
                if not rotmat:
                    root = np_dance[:, :3]  # the root
                    np_dance = np_dance - np.tile(
                        root,
                        (1,
                         24))  # Calculate relative offset with respect to root
                    np_dance[:, :3] = root

                for kk in range((len(np_dance) // move + 1) * move -
                                len(np_dance)):
                ...
                np_dance = None
                dance_data = None
            music_move = external_wav_rate \
                if external_wav is not None else move

            # fully devisable
            for kk in range((len(np_music) // music_move + 1) * music_move -
                            len(np_music)):
