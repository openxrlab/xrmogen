...
def get_music_beat(music_feature_root, key, length=None):
    """
        Fetch music beats from preprocessed music features, represented as bool (True=beats)
        Args:
            music_feature_root: the root folder of preprocessed music features
            key: dance name
...
def calc_beat_align_score(pkl_root, music_feature_root):
    ...
    for pkl in os.listdir(pkl_root):
        ...
        music_beats = get_music_beat(music_feature_root, pkl.split('.')[0] + '.json', length)
