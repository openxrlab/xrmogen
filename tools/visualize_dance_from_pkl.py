...
def visualizeAndWritefromPKL(pkl_root, audio_path=None):
    ...
    for pkl_name in tqdm(os.listdir(pkl_root), desc='Generating Videos'):
        ...
        keypoints3d = Keypoints(
            kps=kps3d_arr_w_conf, mask=kps3d_mask, convention=convention)
        visualize_keypoints3d(
            keypoints=keypoints3d,
            output_path=os.path.join(video_root,
                                     pkl_name.split('.pkl')[0] + '.mp4'))
        ...
        if music_name in music_names:
            ...
            cmd_audio = f'ffmpeg -i {video_root}/{name}.mp4 -i' +\
                        f' {audio_dir_} -map 0:v -map 1:a -c:v' +\
                        ' copy -shortest -y' +\
                        f' {video_root}/{name_w_audio}.mp4 -loglevel quiet'
            os.system(cmd_audio)
